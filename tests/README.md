...
| `test_jobs.sh` | bash + coreutils | job-management parsers that reverse `at_pipe`/`build_next_cmd`: `job_inner_cmd`, `job_summary`, `job_detail` (recovering pane / message count / messages from an `at -c` dump, incl. the printf `%q` → `'\''`-escape round-trip), `atq_time_str` (formatting the GNU/BSD ctime date), plus the `--edit` seed helpers `load_job` (repopulating the scheduling globals), `atq_ctime`, and `ctime_to_epoch` (recovering a pending job's fire time) |
